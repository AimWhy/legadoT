...
书源生态
* 纯JS单文件书源：单个脚本自带 search/getChapters/getContent 等函数，支持发现、登录、段评，配套专用 CodeMirror 编辑器与 .js 导出分享
* 内置 MCP 服务：设置中开启后，AI 助手（Claude Code 等）可直连 App 保存/调试/管理书源、查看 HTTP 日志
* JS引擎：Rhino 更换为 htmlunit-core-js（现 5.3.0），支持 let/const、箭头函数、模板字符串、解构、剩余参数等 ES6+ 语法
* 内置 `CryptoJS`，方便使用网页JS加解密函数
* 书源回调事件系统(callBackJs)：书源可监听17种用户操作事件并执行自定义JS
* 规则能力增强：URL参数支持 `timeout`/`followRedirects`/`resolveIp`，java.get/post/head 新增json字符串请求头重载
...
阅读体验
* 高亮功能：选中文字高亮、高亮规则（正则/关键词）、可组合样式与自定义字体
...
* 定时任务纳入备份范围，恢复去重不再报错，备份支持书架封面和阅读背景图
* 底栏图集支持导入/编辑/导出/分享

调试/开发
* HTTP请求日志：设置中开启后可在日志查看请求概况，点击查看完整请求/响应详情
* Web API 新增书源保存与 HTTP 日志端点

编辑器
* WebView代码编辑器支持自动补全、语法检查、格式化，主题跟随App
