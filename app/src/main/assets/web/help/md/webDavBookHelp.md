...
阅读主要用来看网络小说，同时也支持本地书籍阅读（epub、txt）。

本地书籍在多设备间同步是个难题：换设备后，原设备上的本地书籍需要再次手动导入。阅读没有自己的服务器，但支持 WebDav 备份，可以借助 WebDav 来同步书籍。
...
配置好 WebDav 后，首次进入 WebDav 书籍页没有书籍显示是正常的，因为服务器上还没有上传任何书籍。
...
3. 使用坚果云客户端/自建WebDav服务客户端上传。
   ...
   书籍数量较多时，不建议逐本通过 App 上传，用 WebDav 服务的客户端批量上传更方便。以坚果云为例，进入 [坚果云官网](https://www.jianguoyun.com/d/home#/) 下载对应平台的客户端安装运行，找到 legado/books 目录，这里就是存放书籍的位置，可批量上传到该文件夹下。
