 ...
 * test_tts 与 App 内音色试听同管线(HttpTtsPreview.fetch),不播放、不写缓存;
...
private fun Server.serverAddGetTts() {
    addTool(
        name = "get_tts",
        description = "按 id 取完整朗读引擎 JSON(url 模板/header/jsLib/loginUrl/voices 等全字段)。",
        inputSchema = ToolSchema(
            properties = buildJsonObject {
...
private fun Server.serverAddSaveTts() {
    addTool(
        ...
        description = "推送/覆盖在线朗读引擎。传完整 HttpTTS JSON(get_tts 取回改后再传);id 缺失自动生成。" +
            "url 模板变量:{{speakText}} 文本、{{speakSpeed}} 语速、{{speakVoice}} 音色;" +
