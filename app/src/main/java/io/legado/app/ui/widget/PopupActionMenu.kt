...
class PopupActionMenuBuilder(private val context: Context) {
    ...
    fun item(title: String, value: String, visible: Boolean = true): PopupActionMenuBuilder {
        if (visible) {
            items.add(SelectItem(title, value))
        }
        return this
    }
    ...
    fun show(anchor: View, onClick: (String) -> Unit) {
        show(anchor, 4.dpToPx(), onClick)
    }

    fun show(anchor: View, yOffset: Int = 4.dpToPx(), onClick: (String) -> Unit) {
...
): PopupActionMenuBuilder {
    return PopupActionMenuBuilder(context).apply(block)
}
