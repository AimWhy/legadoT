...
/**
 * 正文选择菜单内置项注册表:稳定 key 与菜单资源 id、标题资源的映射。
 * 动态的第三方"处理文本"(PROCESS_TEXT)项不在此表内。
 */
enum class TextSelectMenuItem(val key: String, val menuId: Int, val titleRes: Int) {
    ...
    companion object {
        ...
        fun menuIdOf(key: String): Int? = byKey[key]?.menuId
    }
}

/**
 * 读取选择菜单配置;首次读取(无配置)时从旧的 expandTextMenu 开关迁移并落盘。
 */
