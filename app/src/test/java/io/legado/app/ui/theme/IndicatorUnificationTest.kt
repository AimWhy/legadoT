 ...
 *
 * Task 6（RotateLoading→LoadingIndicator）与 Task 7（RefreshProgressBar→LinearProgressIndicator，
 * view_loading.xml→LoadingIndicator）对应的断言均在此，三个方法全绿即两个 task 收敛完成。
 */
class IndicatorUnificationTest {

    @Test
    fun `rotate loading is backed by m3 loading indicator`() {
        val src = File("src/main/java/io/legado/app/ui/widget/anima/RotateLoading.kt").readText()
        assertTrue(src.contains("com.google.android.material.loadingindicator.LoadingIndicator"))
    ...
    @Test
    fun `view_loading uses m3 loading indicator`() {
        val xml = File("src/main/res/layout/view_loading.xml").readText()
        assertTrue(xml.contains("com.google.android.material.loadingindicator.LoadingIndicator"))
